...
class DataSource extends \yii\base\Object
{
    ...
    public function search($params = [])
    {
        ...
        if (isset($params[$this->sortParam])) {
            $attrs = explode($this->sortSeparator, $params[$this->sortParam]);
            $orders = isset($params[$this->orderParam]) ? explode($this->sortSeparator, $params[$this->orderParam]) : [];
            foreach ($attrs as $i => $attr) {
                if(isset($this->fieldMap[$attr])){
                    $attr = $this->fieldMap[$attr];
                }
                if (!empty($orders[$i]) && $orders[$i] == 'desc') {
                    $attr = '-' . $attr;
                }
                $attrs[$i] = $attr;
            }
            $params[$this->sortParam] = implode($this->sortSeparator, $attrs);
        }
